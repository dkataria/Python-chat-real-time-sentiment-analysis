...
def ClickAction():
    ...
    #Send my mesage
    print('sending this', EntryText)
    conn.sendall(bytes(EntryText, 'UTF-8'))
...
def updateRating(connStatus, message, RatingChat, RatingMessage, RatingChatColor, RatingMessageColor):
    ...
    ''' Calculate and update the chat and message rating in the chat window '''
...
def GetConnected():
    ...
    global conn
    conn, addr = s.accept()
    LoadConnectionInfo(top.ChatLogBox, 'Connected with: ' + str(addr) + '\n---------------------------------------------------------------')
    SetConnectionInfo(top.UserStatusMessage, top.StatusMessage, str(addr[0]))

def StartReceiving():
    ''' Once connection is established, keep listening on the port for any new messages '''
    while 1:
        try:
            data = conn.recv(1024)
